...
def visualize_reconstruction(target, output, filename,
                             title='', savefig=False, folder_name = 'train'): # loss_func,
    save_dir = 'runs_accuracy/{}_acc/{}.jpg'.format(folder_name, filename) # ../
    ...
    plt.subplot(211)
    plt.plot(target[0, :], 'b', label='original_sample_x', **line_params)
    plt.plot(target[1, :], 'r', label='original_sample_y', **line_params)
    plt.legend()
    plt.subplot(212)
    plt.plot(output[0, :], 'b', label='reconstructed_sample_x', **line_params)
    plt.plot(output[1, :], 'r', label='reconstructed_sample_y', **line_params)
    plt.legend()
    ...
    if savefig:
        ...
        if not os.path.exists('runs_accuracy/{}_acc'.format(folder_name)):
            os.makedirs('runs_accuracy/{}_acc'.format(folder_name))
...
def visualize_pos_to_vel(pos, hz):
    raise Exception('Visualize pos to vel, TODO look because of vel')
